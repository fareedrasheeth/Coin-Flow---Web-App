...
export default function CoinSlotCard({ slot }) {
  const { requestSlotReset, ejectSlotDrawer, closeSlotDrawer } = useCoinFlow();
  ...
  return (
    ...
    >
      ...
      {/* Drawer Action Controls: Manual OPEN / CLOSE / RESET */}
      <div className="mt-4 pt-3 border-t border-gray-100 dark:border-white/10 space-y-2">
        <div className="grid grid-cols-2 gap-2">
          <button
            onClick={() => ejectSlotDrawer(slot.id)}
            className="py-1.5 px-2 rounded-xl bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 text-[11px] font-semibold flex items-center justify-center gap-1 transition-colors"
          >
          ...
          <button
            onClick={() => closeSlotDrawer(slot.id)}
            className="py-1.5 px-2 rounded-xl bg-gray-500/10 hover:bg-gray-500/20 text-gray-300 border border-gray-500/30 text-[11px] font-semibold flex items-center justify-center gap-1 transition-colors"
          >
